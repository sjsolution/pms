                                    ...
                                    <tbody>
                                        @foreach ($propertyrentaldaily as $item)
                                            <tr>
                                                ...
                                                <td>{{ $item->room ? $item->room->room_no : 'N/A' }}</td>
                                                ...
                                                <td><a href="{{ route('admin.propertyrental.show', $item->id) }}"
                                                    ...
                                                    <a class="btn btn-dark " onclick="window.frames['frame_{{$item->id}}'].print()"
                                                        title="Print">
                                            ...
                                            <iframe src="{{ route('admin.propertyrental.pdf', $item->id) }}"
                                                style="display:none;" name="frame_{{$item->id}}"></iframe>
                                        @endforeach
                                    </tbody>
                                </table>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="date" readonly required class="form-control" name="date" value="{{ date('Y-m-d') }}"
                                    id="pay_date">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <select name="payment_type" id="payment_type2" required class="form-control select2"
                                    id="">
...
@section('scripts')
    ...
    localStorage.setItem('AdminLTE:IFrame:Options',JSON.stringify({autoIframeMode:false,autoItemActive:false}))
        ...
        const checkout = (item) => {
            ...
            ele('advance').value = item.paymenttrack_sum_amount != null ?item.paymenttrack_sum_amount : 0;
            ele('propertyrental_id').value = item.id;
            ele('building_id').value = item.building_id;
            var remaining = ele('remaining').value = item.total_amount - item.paymenttrack_sum_amount + parseInt(ele('additional_charges').value);
            if (remaining != 0) {
        ...
        const savepayment = () => {
            ...
            var data = {
                ...
                "payment_type": ele('payment_type2').value,
            };
            $.ajax({
                type: 'POST',
